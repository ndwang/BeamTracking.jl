...
using BeamTracking: Species, massof, chargeof, R_to_beta_gamma, R_to_pc, pc_to_R,
                    ...
                    KernelCall, KernelChain, RefState, launch!, @makekernel,
...
using BeamTracking.RungeKuttaTracking: MultipoleSource, FieldMapSource, eval_em_field, get_curvature, kick_vector
using StaticArrays

@inline function rk4_step_super!(coords, i, s, h, mp_field, fm_field, charge, tilde_m, beta_0, p0c, mc2)
  gx, gy = get_curvature(mp_field)
...
@inline get_g_bend(f::MultipoleSource) = f.gx
@inline get_g_bend(f::FieldMapSource) = f.gx
...
function make_chain(kernel, params)
  ref = RefState(; t_enter=0.0, beta_gamma_enter=1.0, L=1.0, ds_step=0.01)
  kc = KernelChain(Val{1}(), ref)
...
function main()
  ...
  params_mp_phys = (beta_0, tilde_m, charge, p0c, mc2, L, ds_step, n_steps, mp_source)
  params_fm_phys = (beta_0, tilde_m, charge, p0c, mc2, L, ds_step, n_steps, fm_source)
  ...
  for n_particles in [1_000, 10_000, 100_000]
    ...
    if _has_cuda
      ...
      coords_gpu = Coords(state_gpu, v_gpu, nothing, nothing, ())
      ...
      fm_source_gpu = FieldMapSource(fm_gpu, fm_source.z_offset, fm_source.gx, fm_source.gy)
      params_fm_phys_gpu = (beta_0, tilde_m, charge, p0c, mc2, L, ds_step, n_steps, fm_source_gpu)
    ...
    if _has_cuda
      ...
      fm_source_gpu_s = FieldMapSource(fm_gpu_s, fm_source.z_offset, fm_source.gx, fm_source.gy)
