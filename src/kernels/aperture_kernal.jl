...
@makekernel fastgtpsa=true function track_aperture_elliptical!(i, coords::Coords, x1, x2, y1, y2)
  ...
  coords.state[i] = vifelse((r > 1) & (abs(x) ≥ abs(y)) & (x > 0), STATE_LOST_POS_X, coords.state[i])
  coords.state[i] = vifelse((r > 1) & (abs(x) ≥ abs(y)) & (x < 0), STATE_LOST_NEG_X, coords.state[i])
  coords.state[i] = vifelse((r > 1) & (abs(y) ≥ abs(x)) & (y > 0), STATE_LOST_POS_Y, coords.state[i])
  coords.state[i] = vifelse((r > 1) & (abs(y) ≥ abs(x)) & (y < 0), STATE_LOST_NEG_Y, coords.state[i])
end
