...
@inline pure_map(tm::Exact, bunch, mapparams, L) = KernelCall(ExactTracking.map!, (mapparams.transport_map, L))
...
@inline function bend_entrance_fringe(tm::Exact, bunch, bendparams, bmp, L)
  ...
  w_inv = inv_rot_quaternion(0,0,-tilt)
  mm = bmp.order
  kn, ks = get_strengths(bmp, L, bunch.R_ref)
  (mm[1] == 1 && ks[1] ≈ 0) || error("A skew dipole field cannot be used in an exact bend")
  Kn0 = ifelse(mm[1] == 1, kn[1], 0)
  return KernelCall(ExactTracking.linear_bend_fringe!, (e1, Kn0, w, w_inv))
end
