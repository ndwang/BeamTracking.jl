@testset "RungeKuttaTracking" begin
  ...
  @testset "rk4_track! single particle" begin
    ...
    @test isapprox(bunch.coords.v[1, 1], 0.5, rtol=1e-5)  # x = 0.5 * t^2 at t=1
    @test isapprox(bunch.coords.v[1, 2], 1.0, rtol=1e-5)  # px = t at t=1
    @test bunch.coords.v[1, 3] ≈ 0.0
    @test bunch.coords.v[1, 4] ≈ 0.0
    @test bunch.coords.v[1, 5] ≈ 0.0
    @test bunch.coords.v[1, 6] ≈ 0.0
  end

  @testset "rk4_track! with different initial conditions" begin
    # Create a particle with initial position and momentum
    bunch = Bunch(zeros(1, 6))
    bunch.coords.v[1, 1] = 1.0  # x0 = 1.0
    bunch.coords.v[1, 2] = 0.5  # px0 = 0.5
    ...
    @test isapprox(bunch.coords.v[1, 1], expected_x, rtol=1e-5)
    @test isapprox(bunch.coords.v[1, 2], expected_px, rtol=1e-5)
  end
  ...
  @testset "rk4_track! different step sizes" begin
    ...
    RungeKuttaTracking.rk4_track!(1, bunch1.coords, t_span, uniform_field, params, 50)
    RungeKuttaTracking.rk4_track!(1, bunch2.coords, t_span, uniform_field, params, 200)
    ...
    @test isapprox(bunch2.coords.v[1, 1], 0.5, rtol=1e-6)
    @test isapprox(bunch2.coords.v[1, 2], 1.0, rtol=1e-6)
  end
  ...
  @testset "rk4_track! time-varying field" begin
    ...
    @test isapprox(bunch.coords.v[1, 2], 2.0, rtol=1e-5)    # px(2) = 2
    @test isapprox(bunch.coords.v[1, 1], 4 / 3, rtol=1e-5)    # x(2) = 4/3
  end
